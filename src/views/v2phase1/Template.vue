      ...
      <h2>四、岗位薪酬模板（含四·附录：动态薪酬项）</h2>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>个税预扣</td>
                <td>扣缴项</td>
                <td>第2级（每工资条按 3% 预扣，<strong>最后一条工资条不显示</strong>）</td>
              </tr>
              <tr>
                <td rowspan="2">系统项</td>
                <td>预扣合计</td>
                <td>代扣个税</td>
                <td>系统项（月末汇算，<strong>收入项</strong>，=前 N-1 周已扣预扣合计）</td>
              </tr>
              <tr>
                <td>代扣个税（汇算实际税）</td>
                <td>代扣个税</td>
                <td>系统项（月末汇算，<strong>扣缴项</strong>，=百旺实际税）</td>
              </tr>
            </tbody>
          </table>
          <div class="note">
            <strong>福利保障：</strong>社保（仅工伤保险）由<strong>老系统单独记账</strong>，<strong>不进入员工工资条</strong>；商业险<strong>暂不进入系统</strong>（线下购买）<br/>
            <strong>无社保/无公积金代扣项：</strong>非全日制模板中<strong>不包含</strong>代扣社保/代扣公积金系统项——因为公司全额承担，个人不缴<br/>
            <strong>个税汇算（加回 + 扣实际税 模型）：</strong>前 N-1 周每条工资条扣【个税预扣】（3%）；<strong>最后一条工资条不再显示【个税预扣】</strong>（最后一周的预扣内部计算但不计入扣缴项），改为显示<strong>【预扣合计】（收入项，=前 N-1 周已扣预扣合计）</strong> + <strong>【代扣个税】（扣缴项，=百旺实际税）</strong>，实现"加回预扣 + 扣实际税"的清晰表达
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="note">
            ...
            <strong>多主体申报：</strong>按应税金额大的先报，可能跳档<br/>
            <strong>个税汇算（加回 + 扣实际税 模型）：</strong>前 N-1 周每条工资条扣【个税预扣】（3%）；<strong>每主体最后一条工资条不再显示【个税预扣】</strong>（该主体最后一周的预扣内部计算但不计入扣缴项），改为显示<strong>【预扣合计】（收入项，=该主体前 N-1 周已扣预扣合计）</strong> + <strong>【代扣个税】（扣缴项，=百旺该主体实际税）</strong>
          </div>
        </div>
      ...
      <div id="dynamic-items" class="section">
        ...
        <div class="card">
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr>
                <td><strong>预扣合计</strong>（前 N-1 周已扣预扣的"加回"）</td>
                <td>月末百旺汇算时生成：= 前 N-1 周<strong>已扣</strong>的【个税预扣】合计；<strong>最后一周的【个税预扣】内部计算但不显示在扣缴项</strong>（避免与【代扣个税】重复扣减）</td>
                <td>非全日制 / 兼职</td>
                <td>当月<strong>最后一条工资条</strong>的一个<strong>收入项</strong>（"加回"前 N-1 周已预扣的个税，配合【代扣个税】呈现当月真实税负；不单独生成汇算工资条）</td>
              </tr>
              <tr>
                <td><strong>代扣个税</strong>（月末汇算实际税）</td>
                <td>月末百旺汇算：百旺按当月累计应税收入算出的<strong>实际税额</strong></td>
                <td>非全日制 / 兼职</td>
                <td>当月<strong>最后一条工资条</strong>的一个<strong>扣缴项</strong>（不单独生成汇算工资条）</td>
              </tr>
              <tr>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  · <strong>【预扣合计】（收入项）+ 【代扣个税】（扣缴项）</strong>（月末汇算：最后一周不显示【个税预扣】，改为显示【预扣合计】+【代扣个税】）<br/>
                  · <strong>负工资抵扣</strong>（应发 &gt; 0 且有余额时）<br/>
                  · <em>无常乐豆</em>
                </td>
                <td>
                  ① 每周预扣个税 → 累计已预扣合计<br/>
                  ② 月末百旺汇算 → 实际税额；<strong>最后一条工资条不再显示【个税预扣】</strong>（最后一周的预扣内部计算但不计入扣缴项），改为显示<strong>【预扣合计】（收入项，=前 N-1 周已扣预扣合计）</strong> + <strong>【代扣个税】（扣缴项，=百旺实际税）</strong><br/>
                  ③ 工资条应发 &gt; 0 触发负工资抵扣
                </td>
              </tr>
              <tr>
                ...
                <td>
                  · <strong>【预扣合计】（收入项）+ 【代扣个税】（扣缴项）</strong>（月末多主体倒序汇算：每主体最后一条工资条不显示【个税预扣】，改为显示【预扣合计】+【代扣个税】）<br/>
                  · <strong>负工资抵扣</strong>（每主体独立）<br/>
                  · <em>无常乐豆</em>
                </td>
                <td>
                  ① 每周每主体预扣个税<br/>
                  ② 月末按主体应税倒序申报 → 每主体 1 次百旺调用 → 每主体<strong>【预扣合计】（收入项，=该主体前 N-1 周已扣预扣合计）</strong> + <strong>【代扣个税】（扣缴项，=百旺该主体实际税）</strong> → 附加到该主体最后一条工资条<br/>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  · 负工资抵扣 → 减少应发（触发时抵扣）<br/>
                  · <strong>预扣合计 → 增加应发</strong>（收入项，"加回"前 N-1 周已预扣个税）<br/>
                  · <strong>代扣个税（汇算）→ 减少应发</strong>（扣缴项，百旺实际税）<br/>
