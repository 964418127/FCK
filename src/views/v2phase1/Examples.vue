          ...
          <div class="example">
            ...
            <table class="data-table">
              ...
              <tbody>
                ...
                <tr>
                  <td>周工资条4（含汇算）</td>
                  <td>计件提成 +5500；超时加班费 +400；<strong>预扣合计 +492</strong>（=156+147+189，前 3 周已扣预扣合计"加回"）</td>
                  <td><strong>代扣个税 -900</strong>（百旺汇算实际税，最后一周不再显示【个税预扣】）</td>
                  <td>+5492</td>
            ...
            <div class="note">
              <strong>说明：</strong>① 每周工资条各自计算预扣个税（应税收入 × 3%）；② <strong>最后一周工资条不再显示【个税预扣】</strong>（最后一周的预扣内部计算但不计入扣缴项，避免与【代扣个税】重复扣减），改显示<strong>【预扣合计】（收入项，=前 N-1 周已扣预扣合计"加回"）+【代扣个税】（扣缴项，=百旺实际税）</strong>，实现"加回预扣 + 扣实际税"的清晰表达（不单独生成汇算工资条）；③ 实发直接从最后一周应发里加减，不累积为负工资余额；④ <strong>无社保/无公积金代扣项</strong>（公司全额承担，由老系统单独记账）；⑤ <strong>商业险不显示</strong>。
            </div>
          </div>
        </div>
          ...
          <div class="example">
            ...
            <table class="data-table">
              ...
              <tbody>
                ...
                <tr>
                  <td>周工资条4（含汇算）</td>
                  <td>计件提成 +5500；加班 +400；<strong>预扣合计 +495</strong>（=159+147+189，门店A 前 3 周已扣预扣合计"加回"）</td>
                  <td><strong>代扣个税 -1800</strong>（百旺汇算，门店A 倒序申报实际税，最后一周不再显示【个税预扣】）</td>
                  <td>+4595</td>
                ...
                <tr>
                  <td>周工资条4（含汇算）</td>
                  <td>计件提成 +3600；加班 +0；<strong>预扣合计 +270</strong>（=90+84+96，门店B 前 3 周已扣预扣合计"加回"）</td>
                  <td><strong>代扣个税 -1200</strong>（百旺汇算，门店B 倒序申报实际税 = 3000-1800，最后一周不再显示【个税预扣】）</td>
                  <td>+2670</td>
            ...
            <div class="note">
              <strong>说明：</strong>① <strong>1 个门店 = 1 个主体</strong>，多主体（多门店）下每主体独立生成 N 个周工资条（示例：4个），最后一周含汇算项；② 申报顺序按主体应税金额倒序（应税大者先报），每主体<strong>调用百旺 1 次</strong>，传入"截至本主体"的累计应税收入（实现累进税逐级累加），可能跳档；③ <strong>每主体最后一条工资条不再显示【个税预扣】</strong>（最后一周预扣内部计算但不计入扣缴项），改为显示<strong>【预扣合计】（收入项，=该主体前 N-1 周已扣预扣合计"加回"）+【代扣个税】（扣缴项，=百旺该主体实际税）</strong>（不单独生成汇算工资条）；④ 实发直接从每主体最后一周应发里加减，不累积为负工资余额；⑤ 同一员工在不同主体的工资独立发放、互不抵扣；⑥ <strong>无社保/无公积金代扣项</strong>（兼职无社保/公积金）；⑦ <strong>商业险不显示</strong>。
            </div>
          </div>
        </div>
